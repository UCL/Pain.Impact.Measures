...
This dataframe includes Harmonized ELSA data from all waves of ELSA (harmonised g2 dataset). 
...
# counting subjects aged 90 or more
ninetyplus<-H_elsa_w2_ %>%  filter(r2agey >89) 
notyetfifty<-H_elsa_w2_ %>% filter(r2agey <50)

paste("90+ count:", ninetyplus %>% nrow)
paste("<50 count:", notyetfifty %>% nrow)
```


Of this sample of the elsa current wave respondents  
I am excluding the 90+ because elsa do not track age beyond this level and simply classifies age 90+.
...
#sum(H_elsa_w2_$r2agey ==90) 

# removing all participants who younger than 50 or older than 89
H_elsa_w2_ <-H_elsa_w2_ %>% dplyr::filter(is.na(r2agey) | (r2agey <90 & r2agey>49 ))

plotdata0 <- H_elsa_w2_[,c("r2agey","ragender")]# %>% filter(rowSums(H_elsa_w2_[,c("r2agey","ragender")] <0) ==0)
...
ggplot(plotdata0,aes(x=age_group, fill=gender))+
  ...
  ggtitle(paste0("w3 wave: Age Groups by Gender (N = ",nrow(plotdata0),")"))+
  xlab('Age group')+
  ylab('No. of People')
...
```{r, echo=FALSE}
elsa_w2<-read.csv("c:/Users/d.vitali/OneDrive - University College London/CRIISP/00_WP5/00_WP5_data/5050tab/UKDA-5050-tab/tab/wave_1_core_data_v3.tab", sep = "\t")
elsa_w2$helim %>% table
...
Fleishman (1998) and Muthen (1978, 1988).  Arguably ML estimation is a more 
...
The two factors are ected to be correlated. The correlation
assumption is lored via the application of two rotation methods:
